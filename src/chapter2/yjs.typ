...
Yjs là một thư viện CRDT (Conflict-free Replicated Data Type) được viết bằng TypeScript @yjs, thiết kế để hỗ trợ real-time collaboration trên nhiều nền tảng. CRDT cho phép các thay đổi từ nhiều người dùng được tự động hợp nhất mà không cần xung đột, làm cho Yjs trở thành lựa chọn lý tưởng cho các ứng dụng collaborative.

Yjs được sử dụng trong BlockNote @blocknote và nhiều editor khác để cung cấp khả năng collaborative editing. Thư viện này có thể hoạt động với các transport khác nhau như WebSocket, qua các dịch vụ như Hocuspocus @hocuspocus.
...
- Offline Support, người dùng có thể tiếp tục làm việc offline và dữ liệu sẽ đồng bộ khi quay lại online
- Framework Agnostic, Yjs hoạt động độc lập với UI framework, có thể tích hợp với bất kỳ framework nào
- Performance, được tối ưu cho hiệu suất cao, hỗ trợ xử lý các thay đổi lớn một cách hiệu quả
- Ecosystem, hỗ trợ nhiều binding và integration với các tool khác như Tiptap, Monaco Editor, CodeMirror
- Awareness, cung cấp khái niệm "awareness" để hiển thị con trỏ và lựa chọn của người dùng khác
