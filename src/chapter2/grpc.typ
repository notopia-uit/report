...
gRPC là một framework RPC hiện đại được phát triển bởi Google @grpc, sử dụng Protocol Buffers cho định nghĩa service và HTTP/2 cho communication. gRPC được thiết kế để cung cấp hiệu suất cao, latency thấp, và tích hợp tốt với distributed systems.
...
Protocol Buffers là method của Google cho serializing structured data. Protocol Buffers cung cấp compact binary format, cross-language support, backward compatibility, và automatic code generation.

=== gRPC Tooling

- Buf @buf: Build system cho Protocol Buffers, cung cấp code generation, linting, và breaking change detection
