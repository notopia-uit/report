...
//   level 4 — "A.1.1", "A.1.2", … (sub-sub-sections)
//   level 5+ — suppressed (no number shown)
//
...
//
// Figure numbering: "A.1", "A.2", … — letter with dot then sequential counter.
#let appendix(body) = {
  set heading(
    numbering: (..nums) => {
      let n = nums.pos()
